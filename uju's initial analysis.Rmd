...
# pick a small set of key drivers to avoid overfitting
drivers <- c("TP53", "CTNNB1", "AXIN1")

driver_status <- mutation_filtered %>%
  filter(Hugo_Symbol %in% drivers) %>%
  distinct(Patient_ID, Hugo_Symbol) %>%   # one row per gene–patient pair
  mutate(mutated = 1) %>%
  tidyr::pivot_wider(
...
# make sure age and T-stage are usable
merged_data$Age <- as.numeric(merged_data$Diagnosis.Age)
...
# merge with driver mutation flags
analysis_df <- merged_data %>%
  left_join(driver_status, by = "Patient_ID") %>%
  # drop patients with missing key info
  filter(
    !is.na(OS_event),
    !is.na(OS_time),
    !is.na(Age),
    !is.na(T_stage_simple)
  )

# make T-stage a factor with T1 as reference
analysis_df$T_stage_simple <- factor(
  analysis_df$T_stage_simple,
  levels = c("T1", "T2", "T3", "T4")
)

# if any driver columns are missing (no mutations at all), create them as 0
for (g in drivers) {
...
str(analysis_df[, c("OS_event", "Age", "T_stage_simple", drivers)])


# logistic regression:
# OS_event (0/1) ~ Age + T-stage + driver gene mutations
logit_model <- glm(
  OS_event ~ Age + T_stage_simple + TP53 + CTNNB1 + AXIN1,
  data   = analysis_df,
  family = binomial
)
...
accuracy <- mean(pred_class == analysis_df$OS_event, na.rm = TRUE)
accuracy
#sdf
...
#fsalkdfjalskdjfalsdkfjals;dkfja
